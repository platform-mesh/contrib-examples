...
| [`msp-httpbin-operator/`](msp-httpbin-operator/) | HttpBin as an orderable HTTP testing endpoint via the [httpbin-operator](https://github.com/platform-mesh/example-httpbin-operator) — custom `HttpBin` API, status synced back to the consumer | Standalone: kcp as a local host process + a kind cluster for the data plane |
