...
# Apply the on-kind RBAC + the PublishedResource for the HttpBin API. Idempotent (apply).
#
...
PR="${TASKFILE_DIR}/config/syncagent/publishedresource-httpbin.yaml"
...
while [ "${SECONDS}" -lt "${deadline}" ]; do
  schema="$(kubectl --kubeconfig "${KIND_KUBECONFIG}" get publishedresources.syncagent.kcp.io httpbins \
...
if [ -n "${schema}" ]; then
  ...
  echo "    The HttpBin API + permission claims are now in the APIExport 'api-syncagent'."
else
  echo "WARNING: PublishedResource httpbins still has no .status.resourceSchemaName after 120s." >&2
