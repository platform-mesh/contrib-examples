...
# Bound, then asserts that httpbins.orchestrate.platform-mesh.io is served natively in the consumer workspace.
#
# NOTE: this depends on the api-syncagent having already published the HttpBin API into the
...
PROVIDER_WS="${PROVIDER_WS:-root:msp:httpbin-provider}"
...
HTTPBIN_RESOURCE="httpbins.orchestrate.platform-mesh.io"
...
# Assert the HttpBin API is now served in the consumer workspace.
say "asserting $HTTPBIN_RESOURCE is served in $CONSUMER_WS"
served=""
for _ in $(seq 1 30); do
  if kubectl api-resources --api-group=orchestrate.platform-mesh.io 2>/dev/null | grep -qw httpbins; then served=1; break; fi
  sleep 1
done
[ -n "$served" ] || die "$HTTPBIN_RESOURCE is not served in $CONSUMER_WS (did task syncagent:publish populate the APIExport?)"

say "done: $CONSUMER_WS can now order $HTTPBIN_RESOURCE"
