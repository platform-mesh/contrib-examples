...
# Bound, then asserts that certmanager.ca/v1alpha1/Certificate is served in the consumer workspace.
#
# NOTE: this depends on the api-syncagent having already published the certmanager.ca API into the
...
PROVIDER_WS="${PROVIDER_WS:-root:msp:cert-manager-provider}"
...
CERT_RESOURCE="certificates.certmanager.ca"
...
# Assert the certmanager.ca/Certificate API is now served in the consumer workspace.
say "asserting $CERT_RESOURCE is served in $CONSUMER_WS"
served=""
for _ in $(seq 1 30); do
  if kubectl api-resources --api-group=certmanager.ca 2>/dev/null | grep -qw certificates; then served=1; break; fi
  sleep 1
done
[ -n "$served" ] || die "$CERT_RESOURCE is not served in $CONSUMER_WS (did task syncagent:publish populate the APIExport?)"

say "done: $CONSUMER_WS can now order $CERT_RESOURCE"
