...
# Apply the on-kind RBAC + the PublishedResource for the certmanager.ca/Certificate API. Idempotent (apply).
#
...
PR="${TASKFILE_DIR}/config/syncagent/publishedresource-certificate.yaml"
...
while [ "${SECONDS}" -lt "${deadline}" ]; do
  schema="$(kubectl --kubeconfig "${KIND_KUBECONFIG}" get publishedresources.syncagent.kcp.io cert-manager-certificates \
...
if [ -n "${schema}" ]; then
  ...
  echo "    The certmanager.ca/Certificate API + permission claims are now in the APIExport 'api-syncagent'."
else
  echo "WARNING: PublishedResource cert-manager-certificates still has no .status.resourceSchemaName after 120s." >&2
