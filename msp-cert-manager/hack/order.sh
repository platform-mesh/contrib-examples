#!/usr/bin/env bash
# hack/order.sh — owner: cert-manager-expert
# Apply the Certificate order into the kcp consumer workspace.
...
MANIFEST="${TASKFILE_DIR}/config/samples/order-certificate.yaml"
...
echo "==> Applying Certificate order from ${MANIFEST}..."
kubectl --kubeconfig "${KCP_KUBECONFIG}" apply -f "${MANIFEST}"

echo "==> Order submitted. Certificate 'my-cert' is now in workspace ${CONSUMER_WS}."
echo "    api-syncagent will sync it to kind; watch with:"
echo "    kubectl --kubeconfig \"\${KCP_KUBECONFIG}\" get certificate.certmanager.ca my-cert -n default -w"
